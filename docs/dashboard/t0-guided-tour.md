...
- **ID card** (`IdCardStep`): the capstone. Locked until every program video is
  ...
  `idCardUrl` isn't set yet. The URL comes from the **live admissions
  `student-status` API** (`idCard.url`) on every request — not from
  `user_batch_admission_data.id_card_url`, which is written at enrolment but
  never read back. The LMS never generates the card.

  The capstone is hidden entirely (neither locked nor unlocked) in three cases:
  the lite (non-T0) flow, clients that don't issue an LMS ID card
  (`idCardApplicable: false` — IITJ), and programs where admissions reports
  `idCard.idCardConfigured: false`. That last one matters: without it, a program
  with no admit-card template configured would sit on "being generated … within
  30 minutes" forever, since no URL is ever coming. Note the "being generated"
  state still doubles as the catch-all for transient failures (missing
  `ADMISSIONS_API_*` env, admissions 4xx/5xx, 15s timeout) — check the
  `[student-status]` logs to tell those apart.
