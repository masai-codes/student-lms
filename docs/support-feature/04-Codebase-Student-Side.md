...
### 2.1 `tickets` *(schema.prisma:2457)*
...
### 2.2 `comments` — the conversation thread *(schema.prisma:724)*
...
### 2.4 `user_callback_tickets` — callbacks *(schema.prisma:3001)*
...
### 2.5 `interactions` + `interaction_messages` — chatbot *(schema.prisma:3110 / 3126)*
...
> logic does**.
>
> **Important architectural finding:** the support feature is *not* self-contained. It
...
> services your support UI merely *consumes* — implement them only if the new repo
...
- **Implement Group A** (16 support-core operations + callback config) — this *is* the
  support backend.
...
> The earlier draft of this doc (§3) listed the *primary* operations; this §10 is the
