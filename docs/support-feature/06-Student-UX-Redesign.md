...
> into a **top-notch, responsive experience** that feels great on a phone *and* on a wide
...
**Guiding principle:** *Answer before ticket, conversation over form, one clear next step.*
...
Because creation almost always comes *from* an FAQ, lean into that:
...
- **Gates handled gracefully**: if legal-agreement/active-section blocks creation, show the reason *before* they type, with the unblock CTA — never let them write a message then reject it.
...
- **Success:** confirmations with the *next* expectation (TAT, callback window).
