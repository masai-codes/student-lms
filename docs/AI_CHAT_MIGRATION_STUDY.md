...
**Source of truth for the active thread = [`useLectureAIChat`](hooks/useLectureAIChat.ts) — plain React `useState`/`useRef`, NOT React Query.** The history _list_ is the only React-Query-backed piece.
...
- Owns only **UI orchestration** state (drawer open, history open, feedback modal); delegates _all_ chat/streaming to `useLectureAIChat`. Clean separation → easy port.
