...
```bash
npm run test -- src/server/learn/utils/__tests__/resolveAssignmentPhase.test.ts src/server/learn/utils/__tests__/buildAssignmentDetailPayload.test.ts src/components/features/learn/LearnPageDetails/assignment/shared/__tests__/assignmentPhaseCopy.test.ts
npm run typecheck
```
...
- Confirm hero state, header meta, instructions block, and discussions panel render.
