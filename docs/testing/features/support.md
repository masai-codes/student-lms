...
- **Ticket title generation**: on create, `resolveTicketTitle` tries OpenAI (`gpt-4.1-mini`, 1.5s timeout) when `OPENAI_API_KEY` is set and `SUPPORT_AI_TITLES` is not `false`; otherwise falls through deterministically: entity title + subcategory → first message line → category + subcategory → `"Support request"`. Stores `title_source` in `tickets.data`. Entity titles load via `fetchEntityTitleForTicket` when `entity_id` is present.
