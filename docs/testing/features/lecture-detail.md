...
- AI "Ask" chat (desktop): the lecture video always stays **full width** — the AI chat no longer opens as a sidebar that shrinks it. Instead a floating "Ask" launcher (`LectureFloatingChat`) sits bottom-right and opens a docked popup the user can **maximize** (expand toggle in the panel header). Open state is shared via `LectureSplitChatContext` / `useLectureSplitChatOpen` (localStorage `lecture-split-chat-open`). Page-level launcher renders via a `document.body` portal (variant `viewport`, hidden by `useLectureVideoFullscreenActive`); while the video is in browser fullscreen the launcher/popup render **inside** the fullscreen root (variant `contained`, in `LectureReactPlayer`) so Ask stays reachable. Mobile keeps the existing below-hero dock (`LectureAiChatExperience variant="mobile-dock"`). The old in-toolbar `LectureVideoAskAiPill` and the shrink-to-sidebar layout are removed.
