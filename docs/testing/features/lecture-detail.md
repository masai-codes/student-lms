...
- AI "Ask" chat (desktop): opened from the in-player controls "Ask AI" pill (`LectureVideoControlsToolbar`, dark-glass pill shared with `AskAiPill`). Inline, the chat docks to the **right of the video as a resizable split** (`LectureDesktopVideoStage` + `LectureChatResizeHandle`): a prominent draggable divider (pointer + Arrow keys) lets the learner set the video/chat width split, clamped so each keeps a usable width and persisted to localStorage `lecture-chat-width` (`useLectureChatWidth`). Open state is shared via `LectureSplitChatContext` / `useLectureSplitChatOpen` (localStorage `lecture-split-chat-open`). While the video is in browser fullscreen the chat renders **inside** the fullscreen root as a docked popup (`LectureFloatingChat variant="contained"`, in `LectureReactPlayer`); its floating launcher pill is disabled (`showLauncher={false}`) since the in-player pill is the single entry point. Mobile keeps the below-hero dock (`LectureAiChatExperience variant="mobile-dock"`). The old `LectureVideoAskAiPill` and the fixed shrink-to-sidebar layout are removed.
...
- `src/components/features/lecture-ai-chat/components/__tests__/LectureFloatingChat.test.tsx` — launcher open + analytics, docked/maximized popup, close/expand tracking, contained (fullscreen) variant, close-transition unmount
