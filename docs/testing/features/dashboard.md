...
- `DASH-003` - Module: `WelcomeBannerCarousel` - Case: renders nothing when empty; single banner shows title/description with no arrows/dots; >1 shows bounded arrows (prev disabled on first, next on last) + dots and advances on click; `cta_url` routing (`/` internal same-tab, full URL new tab, none → `/changemakers-circle`); pushes the `l_dashboard_banner_carousel_<key>_id_<id>` GTM event on click - Status: Covered
...
- `DASH-008` - Module: `AnnouncementsPanel` - Case: loading spinner + error message from query state; empty state (header + View All + "No announcements yet"); rows with the "For you" badge only on messages; message → `/messages/$id`, announcement → `/announcements/$id` - Status: Covered
