...
> explains the overall flow and *why* the two conventions exist.
...
**The READ primitive.** Normalizes whatever mysql2 hands back — a naive string *or*
...
Parses a **UTC** server timestamp *with* the far-future→local fallback (default
...
Detail-page range *with the year* and a tz label, in the viewer's zone.
