...
| `section-dropdown-batch`           | Batch with `meta.showSectionDropdown = true` and 3 sections, each with a live lecture, video lecture, and assignment — exercises the `/learn` section (Course) filter dropdown                                                                                                                                                                                                                                              |
| `discussions-cancelled-enrollment` | Student enrolled in two batches — `batch_user.meta.batchEnrolmentCancelled = true` on batch A (their `section_user` row there stays active), healthy/active on batch B — with each batch's lecture carrying public discussions from multiple people (a batch-local author plus the multi-batch student themself) — exercises `listLearnDiscussionsForBatch` hiding batch A's discussions while batch B's stay fully visible |
| `dashboard-home`                   | Dashboard: My Schedule, Pending Tasks, Announcements, Product Updates (edge cases)                                                                                                                                                                                                                                                                                                                                          |
| `profile-page`                     | `/profile` test bed: student in two batches (two student codes), 2 modules each with one earned + one locked badge (program → module achievements grouping), one pending acknowledgement, 3 device sessions (laptop/phone/tablet), and `full_fees_paid` + an admission row so the Student Kit and My Invoices tabs unlock. Those two tabs proxy the external Admissions API, so the seed only unlocks them.                 |
