...
| `discussions-cancelled-enrollment` | Student with `batch_user.meta.batchEnrolmentCancelled = true` on a batch where their `section_user` row is still active, plus a second student's public discussion on a lecture in that batch — exercises `listLearnDiscussionsForBatch` hiding a cancelled batch's discussions |
