...
const [bi] = await conn.execute(
  "SELECT item, value FROM batch_info WHERE batch_id = 140 AND item LIKE '%code%'",
)
console.log('batch_info code for batch 140:', bi)

// Check if profiles has roll_number or student_code field
const [pr] = await conn.execute(
  "SELECT column_name FROM information_schema.columns WHERE table_schema = 'lms_dev_db' AND column_name LIKE '%code%' OR column_name LIKE '%roll%'",
)
console.log(
  'columns with code/roll:',
  pr.map((r) => r.column_name + ' in ' + r.table_name ?? ''),
)
