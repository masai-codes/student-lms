...
function runStream(response: Response): Promise<StreamResult> {
  ...
  return new Promise((resolve) => {
    streamSubmitInterviewTurn(
      ...
      {
        onQuestionDelta: (text) => deltas.push(text),
...
describe('streamSubmitInterviewTurn', () => {
  ...
  it('dispatches question-delta events then done with the result', async () => {
    const response = streamResponse([
      sse({ type: 'question-delta', text: 'How do ' }),
      sse(
        { type: 'question-delta', text: 'you handle collisions?' },
        {
          type: 'done',
          result: {
            status: 'in_progress',
            transcript: 'a hash map',
    ...
    expect(result.deltas).toEqual(['How do ', 'you handle collisions?'])
  ...
  it('surfaces a terminal error event emitted mid-stream', async () => {
    const response = streamResponse([
      sse({ type: 'question-delta', text: 'partial' }),
      sse({ type: 'error', code: 'INTERVIEW_TRANSCRIPT_EMPTY' }),
    ])

    const result = await runStream(response)
    expect(result.status).toBe('error')
    expect(result.code).toBe('INTERVIEW_TRANSCRIPT_EMPTY')
  ...
  it('sends the answer as multipart form data with same-origin credentials', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      streamResponse([
        sse({
          type: 'done',
          result: { status: 'completed', transcript: 'x', report: {} },
        }),
      ]),
    )
