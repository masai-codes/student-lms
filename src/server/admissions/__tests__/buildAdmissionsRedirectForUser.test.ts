      ...
      select: () => ({
        from: () => ({
          where: () => ({
            limit: () =>
              Promise.resolve(
                call++ === 0 ? hoisted.userRows : hoisted.profileRows,
              ),
          }),
        }),
      }),
...
vi.mock('../createAdmissionsSsoToken', () => ({
  buildAdmissionsSsoUrl: hoisted.buildUrl,
}))
...
describe('buildAdmissionsRedirectForUser', () => {
  ...
  it('returns null when the SSO signer throws (unconfigured)', async () => {
    hoisted.buildUrl.mockImplementation(() => {
      throw new Error('ADMISSIONS_SSO_SECRET is not configured')
    })
    expect(
      await buildAdmissionsRedirectForUser(1, 'https://redirect'),
    ).toBeNull()
  })
})
