...
describe('notifyDiscussionReplyViaExperienceApi', () => {
  it('skips the call when base url or secret is missing', async () => {
    ...
    const { notifyDiscussionReplyViaExperienceApi } =
      await import('../triggerExperienceApiCommunityNotify')
  ...
  it('posts to the trimmed base url with club_id and the default type', async () => {
    ...
    expect(url).toBe(
      'http://localhost:4000/internal/community-masaiverse/notify/post-reply',
    )
    expect(
      (init.headers as Record<string, string>)['x-community-masaiverse-secret'],
    ).toBe('sekret')
