...
function buildScopedObjectKey(scope: S3UploadScope, fileName: string, contentType: string): string {
...
export async function generatePresignedPostPolicy(input: {
  ...
  const bucketPath = buildScopedObjectKey(input.scope, input.fileName, contentType)
