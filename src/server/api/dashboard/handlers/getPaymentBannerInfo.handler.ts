...
export async function handleGetPaymentBannerInfo(request: Request): Promise<Response> {
  try {
    ...
    if (!isApiError(error)) {
      ...
      return mapThrownErrorToResponse(new Error('SERVER_ERROR_FETCHING_PAYMENT_BANNER'))
    }
