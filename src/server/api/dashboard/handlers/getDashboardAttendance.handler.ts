...
export async function handleGetDashboardAttendance(request: Request): Promise<Response> {
  try {
    ...
    if (!isApiError(error)) {
      ...
      return mapThrownErrorToResponse(new Error('SERVER_ERROR_FETCHING_DASHBOARD_ATTENDANCE'))
    }
