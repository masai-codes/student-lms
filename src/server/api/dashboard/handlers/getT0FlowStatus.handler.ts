...
export async function handleGetT0FlowStatus(request: Request): Promise<Response> {
  try {
    ...
    if (!isApiError(error)) {
      ...
      return mapThrownErrorToResponse(new Error('SERVER_ERROR_FETCHING_T0_FLOW_STATUS'))
    }
