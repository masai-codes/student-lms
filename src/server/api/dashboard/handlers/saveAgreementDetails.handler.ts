...
export async function handleSaveAgreementDetails(
  ...
  try {
    ...
    const ipAddress =
      ...
      (data.clientIp && data.clientIp !== '127.0.0.1' && data.clientIp !== '::1' ? data.clientIp : null) ??
      'unknown'

    await saveAgreementDetails(id, userId, data, ipAddress, deviceDetails)
    return jsonOk({ success: true, ipAddress, referenceNumber: `TC-${userId}-section_${id}` })
  } catch (error) {
    if (!isApiError(error)) console.error('Failed to save agreement details', error)
