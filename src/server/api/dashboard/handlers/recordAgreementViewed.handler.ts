...
export async function handleRecordAgreementViewed(request: Request): Promise<Response> {
  try {
    ...
    if (!isApiError(error)) {
      ...
      return mapThrownErrorToResponse(new Error('SERVER_ERROR_RECORDING_AGREEMENT_VIEW'))
    }
