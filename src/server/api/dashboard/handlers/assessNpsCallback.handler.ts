...
export async function handleAssessNpsCallbackRequest(request: Request): Promise<Response> {
  console.log('[assess-nps-callback] incoming request', request.method, request.url)
  console.log('[assess-nps-callback] headers:', JSON.stringify(Object.fromEntries(request.headers.entries())))
  try {
    ...
    console.log('[assess-nps-callback] parsed body:', JSON.stringify(body, null, 2))
