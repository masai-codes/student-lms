...
const certData = {
  ...
  studentCode: 'riya1',
  address: '12 MG Rd, Bengaluru — a deliberately long address to exercise line wrapping on the certificate page',
  program: 'MERN',
...
describe('buildAgreementPdf', () => {
  it('merges the agreement doc and appends a certificate page (logo optional)', async () => {
    ...
    const out = await buildAgreementPdf([{ heading: 'Program', bytes: docBytes }], certData, null)
  ...
  it('skips an unloadable agreement doc but still produces the certificate', async () => {
    const out = await buildAgreementPdf([{ heading: 'Broken', bytes: new ArrayBuffer(4) }], certData, null)
    const result = await PDFDocument.load(out)
