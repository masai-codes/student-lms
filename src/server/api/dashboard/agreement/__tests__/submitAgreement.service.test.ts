...
vi.mock('@/db', () => ({
  db: {
    select: () => ({ from: () => ({ where: () => ({ limit: () => Promise.resolve(hoisted.selectQueue.shift() ?? []) }) }) }),
...
const SECTION = {
  name: 'Enrolment',
  batch_id: 5,
  agreements: JSON.stringify({ program_agreement: { heading: 'Program', pdfUrl: 'https://x/p.pdf' } }),
}
...
describe('submitAgreement', () => {
  it('rejects when the user is not enrolled in the section', async () => {
    hoisted.selectQueue = [[]]
    await expect(submitAgreement(1, 7, '1.2.3.4')).rejects.toMatchObject({ code: 'NOT_ENROLLED_IN_SECTION' })
  ...
  it('generates the PDF, uploads it, and marks the agreement signed', async () => {
    ...
    expect(hoisted.uploadS3).toHaveBeenCalledWith(expect.objectContaining({ contentType: 'application/pdf', ext: 'pdf' }))
