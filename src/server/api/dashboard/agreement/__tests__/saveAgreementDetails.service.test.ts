...
vi.mock('@/db', () => ({
  db: {
    ...
    update: () => ({ set: (v: unknown) => { hoisted.updateSet(v); return { where: () => Promise.resolve() } } }),
    insert: () => ({ values: (v: unknown) => { hoisted.insertValues(v); return Promise.resolve() } }),
  },
}))
...
describe('saveAgreementDetails', () => {
  it('merges values into an existing profile and stamps a reference number', async () => {
    hoisted.selectRows = [{ id: 3, legalData: { agreements: { section_7: { formDetailCreateTime: 'earlier' } } } }]

    const result = await saveAgreementDetails(1, 7, { name: 'Riya', panNumber: 'ABCDE1234F', junk: 'x' } as never)

    expect(result.referenceNumber).toBe('TC-1-section_7')
    expect(result.savedValues).toMatchObject({ name: 'Riya', panNumber: 'ABCDE1234F' })
    expect(result.savedValues).not.toHaveProperty('junk') // whitelisted to known fields

    const written = hoisted.updateSet.mock.calls[0][0] as { legalData: { agreements: { section_7: Record<string, unknown> } } }
  ...
  it('creates a profile row when none exists', async () => {
    ...
    const inserted = hoisted.insertValues.mock.calls[0][0] as { userId: number; legalData: { agreements: Record<string, unknown> } }
