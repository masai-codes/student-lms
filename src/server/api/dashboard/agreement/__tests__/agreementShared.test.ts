...
import { buildAgreementSteps, buildReferenceNumber, computeAgreementCountdown, hasSignableAgreement } from '../agreementShared'
...
describe('computeAgreementCountdown', () => {
  ...
  it('shows the full window in days when never viewed', () => {
    expect(computeAgreementCountdown(null)).toEqual({
      daysSinceFirstView: 0, daysLeft: 7, hoursLeft: null, isClosable: true,
    })
  })

  it('counts down in days while more than a day remains', () => {
    expect(computeAgreementCountdown(viewedAgo(2 * DAY_MS))).toEqual({
      daysSinceFirstView: 2, daysLeft: 5, hoursLeft: null, isClosable: true,
    })
  })

  it('switches to hours when under a day remains', () => {
    // 6d19h elapsed → ~5h left in the 7-day window.
    expect(computeAgreementCountdown(viewedAgo(7 * DAY_MS - 5 * HOUR_MS))).toEqual({
      daysSinceFirstView: 6, daysLeft: 0, hoursLeft: 5, isClosable: true,
    })
  })

  it('reports at least one hour in the final stretch', () => {
    expect(computeAgreementCountdown(viewedAgo(7 * DAY_MS - 10 * 60 * 1000)).hoursLeft).toBe(1)
  })

  it('is no longer closable once the window has elapsed', () => {
    expect(computeAgreementCountdown(viewedAgo(8 * DAY_MS))).toEqual({
      daysSinceFirstView: 8, daysLeft: 0, hoursLeft: null, isClosable: false,
    })
  })
