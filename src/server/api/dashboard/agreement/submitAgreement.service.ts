...
export async function submitAgreement(
  ...
  const [sectionRow] = normalizeRows<{ name: string; batch_id: number; agreements: string | null }>(
    await db.execute(sql`
      SELECT name, batch_id, settings->>'$.agreements' AS agreements
      FROM sections WHERE id = ${sectionId} LIMIT 1
    `),
  )
  const steps = sectionRow ? buildAgreementSteps(asRecord(safeJson(sectionRow.agreements))) : []
  if (!steps.length) throw new ApiError(400, 'NO_AGREEMENT_FOR_SECTION')

  const [[batch], [user], [profile]] = await Promise.all([
    db.select({ name: batches.name, program: batches.program }).from(batches).where(eq(batches.id, sectionRow.batch_id)).limit(1),
    db.select({ name: users.name, email: users.email, username: users.username }).from(users).where(eq(users.id, userId)).limit(1),
    db.select({ id: profiles.id, legalData: profiles.legalData })
  ...
  const pdfBytes = await buildAgreementPdf(docs, {
    ...
    viewed: (existing['viewTime'] as string | undefined) ?? (existing['formDetailCreateTime'] as string | undefined) ?? nowIso,
    signed: nowIso,
    ipAddress,
  ...
  await db
    .update(profiles)
    .set({ legalData: { ...legalData, agreements: { ...agreements, [key]: merged } } })
