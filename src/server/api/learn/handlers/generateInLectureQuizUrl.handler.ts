...
import { generateInLectureQuizUrl } from '@/server/learn/services/generateInLectureQuizUrl.service'

const bodySchema = z.object({
  assessmentTemplateId: z.string().min(1),
})

export async function handleGenerateInLectureQuizUrl(
  request: Request,
  ...
  try {
    ...
    const result = await generateInLectureQuizUrl({
      userId,
      lectureId,
