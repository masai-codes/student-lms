...
const REJECTED_AI_TITLES = new Set([
  'na', 'n/a', 'help', 'issue', 'question', 'support ticket', 'support request', 'ticket',
])
...
export function sanitizeAndValidateAiTitle(raw: string | null | undefined): string | null {
...
function buildAiPrompt(input: ResolveTicketTitleInput): {
  ...
  const plainMessage = plainTextFromHtml(input.message).slice(0, MAX_AI_PROMPT_CHARS)
