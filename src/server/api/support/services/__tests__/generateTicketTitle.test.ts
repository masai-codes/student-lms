...
describe('generateTicketTitle.service', () => {
  ...
  describe('humanizeTicketSlug', () => {
    it('replaces hyphens and underscores with spaces', async () => {
      const { humanizeTicketSlug } = await import('../generateTicketTitle.service')
  ...
  describe('titleFromCategorySubcategory', () => {
    it('joins category and subcategory with an en dash', async () => {
      const { titleFromCategorySubcategory } = await import(
        '../generateTicketTitle.service'
      )
  ...
  describe('titleFromMessage', () => {
    it('uses the first non-empty line and caps length', async () => {
      const { titleFromMessage } = await import('../generateTicketTitle.service')
      expect(
        titleFromMessage(
  ...
  describe('sanitizeAndValidateAiTitle', () => {
    it('accepts a valid title and strips wrapping quotes', async () => {
      const { sanitizeAndValidateAiTitle } = await import(
        '../generateTicketTitle.service'
      )
      expect(
    ...
    it('rejects junk and overlong titles', async () => {
      ...
      expect(
        sanitizeAndValidateAiTitle('one two three four five six seven eight nine ten eleven twelve thirteen'),
      ).toBeNull()
    })
  })
  ...
  describe('resolveTicketTitle', () => {
    ...
    it('falls back to the message line when AI fails and no entity is present', async () => {
      ...
      const { resolveTicketTitle } = await import('../generateTicketTitle.service')
      const result = await resolveTicketTitle({
        message: 'Can you please share the html and colab files for this session.',
        category: 'others',
        subCategory: null,
      })
  ...
  describe('fetchEntityTitleForTicket', () => {
    it('returns a lecture title when the student can access the section', async () => {
      ...
      const { fetchEntityTitleForTicket } = await import(
        '../fetchEntityTitleForTicket.service'
      )
      ...
      expect(hoisted.ensureUserCanAccessLearnHubEntity).toHaveBeenCalledWith(1, 9)
    })
