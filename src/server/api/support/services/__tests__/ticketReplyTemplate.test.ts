...
describe('buildFirstTemplateResponse', () => {
  ...
  it('uses the ops L1 fallback title and the legacy body for a non-assignment ticket', async () => {
    ...
    const { buildFirstTemplateResponse } = await import('../ticketReplyTemplate')
    ...
    expect(message).toContain('Regards,<br/>Program Co-ordinator<br/>Student Experience Team')
  ...
  it('uses the curriculum title and never a phone for assignment-track tickets', async () => {
    ...
    expect(message).toContain('Regards,<br/>Curriculum Co-ordinator<br/>Student Experience Team')
    expect(message).not.toContain('99999')
  })

  it('prefers the batch opsRoleTitles override and includes the L1 phone', async () => {
    mockSelect([
      { settings: { opsRoleTitles: { l1: 'Batch Buddy' }, phNumbers: { ph_l1: '1800-123' } } },
    ])
    const { buildFirstTemplateResponse } = await import('../ticketReplyTemplate')
    ...
    expect(message).toContain('Regards,<br/>Batch Buddy<br/>1800-123<br/>Student Experience Team')
  })
  ...
  it('uses the assignee name when showAdminNameInTicketReply is set', async () => {
    ...
    expect(message).toContain('Regards,<br/>Asha Rao<br/>Student Experience Team')
  })
})
