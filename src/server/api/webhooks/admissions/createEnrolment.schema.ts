...
export const createEnrolmentSchema = z
  .object({
    ...
    seat_blocking_fees_amount: z.number().nonnegative().optional(),
    seat_blocking_fees_paid_date: z.string().trim().min(1).optional(),
    seat_blocking_fees_invoice: z.string().trim().min(1).optional(),
    student_kit_exists: z.boolean().optional(),
