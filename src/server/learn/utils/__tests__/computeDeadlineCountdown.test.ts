...
const at = (offsetMs: number) => new Date(NOW + offsetMs).toISOString().replace('T', ' ').slice(0, 19)
...
describe('computeDeadlineCountdown', () => {
  it('counts whole days remaining when >= 1 day', () => {
    ...
    const result = computeDeadlineCountdown(deadline, new Date(deadline).getTime() - (3 * DAY + HOUR))
    expect(result).toEqual({ totalMs: 3 * DAY + HOUR, label: '3 days remaining' })
  })

  it('uses singular "1 day remaining"', () => {
    const deadline = at(0)
    expect(computeDeadlineCountdown(deadline, new Date(deadline).getTime() - (DAY + HOUR))?.label).toBe(
      '1 day remaining',
    )
  })
  ...
  it('returns null with no deadline or once it has passed', () => {
    ...
    const deadline = at(0)
    expect(computeDeadlineCountdown(deadline, new Date(deadline).getTime() + HOUR)).toBeNull()
  })
})
