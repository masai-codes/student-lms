...
const DEFAULT_CERTIFICATE_TEMPLATES_S3_URI = 's3://experience-api-production/certificates/'
...
function resolveCertificateS3Credentials() {
  ...
  if (accessKeyId && secretAccessKey) return { accessKeyId, secretAccessKey, ...(sessionToken ? { sessionToken } : {}) }
  return undefined
}
...
export function resolveCertificateS3Config(): {
  ...
  return { certificateTemplatesS3Uri, region, ...(credentials !== undefined ? { credentials } : {}) }
}
