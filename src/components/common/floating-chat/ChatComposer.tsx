...
export function ChatComposer({
  ...
  const canSend = (message.trim().length > 0 || files.length > 0 || !!selectedTopic) && !uploading && !disabled
  ...
  const handleFormat = (
    format: 'bold' | 'italic' | 'underline' | 'bullet' | 'number' | 'code' | 'link',
  ) => {
  ...
  const tabButtonClass = (active: boolean) =>
    cn(
      ...
      active ? 'bg-[#f0f0fd] text-[#4b4396]' : 'text-[#9496ab] hover:text-[#62647d]',
    )
        ...
        {selectedTopic && (
          <div className="px-3 pt-3 flex items-start justify-between gap-2">
            <div className="inline-flex items-center gap-1.5 bg-[#f0f0fd] px-3 py-1.5 rounded-[10px] text-[#4b4396] max-w-[90%]">
              <span className="text-[11.5px] font-bold uppercase tracking-wide shrink-0">Topic:</span>
              <span className="text-[13px] font-medium truncate">{selectedTopic}</span>
            </div>
        ...
        {isPreview ? (
          ...
          >
            {message.trim().length > 0 ? (
              <SupportMarkdown className="text-[13.6px] text-[#15162c]">{message}</SupportMarkdown>
            ) : (
              <p className="text-[13.6px] text-[#9496ab]">Nothing to preview yet.</p>
            )}
          </div>
        ) : (
          <textarea
