...
type AttachmentType = 'image' | 'video' | 'pdf' | 'archive' | 'code' | 'text' | 'file'

function detectType(ext: string): AttachmentType {
  if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp', 'avif'].includes(ext)) return 'image'
  if (['mp4', 'mov', 'webm', 'avi', 'mkv', 'flv', 'm4v'].includes(ext)) return 'video'
  if (ext === 'pdf') return 'pdf'
  if (['zip', 'rar', 'tar', 'gz', '7z'].includes(ext)) return 'archive'
  if (['js', 'ts', 'tsx', 'jsx', 'py', 'java', 'c', 'cpp', 'rb', 'go', 'rs', 'html', 'css', 'json', 'xml', 'yaml', 'yml'].includes(ext)) return 'code'
...
function ImageCard({ url, label, variant }: { url: string; label: string; variant: 'user' | 'agent' }) {
      ...
      <div className="mt-1.5 group relative">
        <button
          ...
          className={`block w-full text-left rounded-[10px] overflow-hidden transition-all duration-200 hover:scale-[1.01] active:scale-[0.99] ${
            variant === 'user' ? 'ring-1 ring-white/20' : 'ring-1 ring-[#e9e9f3]'
          }`}
        ...
        {label && label !== url && (
          <p className={`mt-1 text-[11px] truncate ${variant === 'user' ? 'text-white/60' : 'text-[#9496ab]'}`}>{label}</p>
        )}
      </div>
    </>
  )
...
function VideoCard({ url, label, variant }: { url: string; label: string; variant: 'user' | 'agent' }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-[13px] font-semibold truncate leading-snug ${variant === 'user' ? 'text-white' : 'text-[#15162c]'}`}>
          {label}
        </p>
        <p className={`text-[11px] mt-0.5 flex items-center gap-1 ${variant === 'user' ? 'text-white/60' : 'text-[#9496ab]'}`}>
      ...
      <a
        ...
        className={`shrink-0 p-1.5 rounded-full transition-colors ${
          variant === 'user' ? 'text-white/60 hover:text-white hover:bg-white/10' : 'text-[#9496ab] hover:text-[#15162c] hover:bg-[#e0e0ef]'
        }`}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className={`text-[11px] mt-0.5 ${variant === 'user' ? 'text-white/60' : 'text-[#9496ab]'}`}>
          {typeLabel}
        </p>
      </div>

      <div className={`shrink-0 p-1.5 rounded-full transition-colors ${
        variant === 'user' ? 'text-white/60 group-hover:text-white' : 'text-[#9496ab] group-hover:text-[#15162c]'
      }`}>
 ...
 */
export function AttachmentCard({ url, label = '', variant = 'agent' }: AttachmentCardProps) {
