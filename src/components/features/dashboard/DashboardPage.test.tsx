...
describe('DashboardPage', () => {
  it('greets the user and renders the API-driven banner, announcements, updates + support', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('dashboard-welcome-name').textContent,
      ).toContain('Suryakumar'),
    )

    await waitFor(() =>
      expect(screen.getByText('Live referral banner')).toBeTruthy(),
    )
  ...
  it('shows per-card loading and hides the support/banner cards while loading', () => {
    ...
    expect(
      screen.getByTestId('dashboard-announcements-panel-loading'),
    ).toBeTruthy()
    expect(
      screen.getByTestId('dashboard-product-updates-panel-loading'),
    ).toBeTruthy()
