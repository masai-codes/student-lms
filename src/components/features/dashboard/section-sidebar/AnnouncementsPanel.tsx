...
export function AnnouncementsPanel({ announcements }: AnnouncementsPanelProps) {
  return (
    <SidebarPanel
      ...
      action={<SidebarPanelLink label="View all" testId="dashboard-announcements-view-all" />}
    >
      {announcements.length === 0 ? (
        <p data-testid="dashboard-announcements-empty" className="text-sm text-gray-400">
...
function AnnouncementRow({ announcement }: { announcement: Announcement }) {
  return (
    <div
      data-testid={`dashboard-announcement-item-${announcement.id}`}
      className="rounded-xl border border-gray-200 p-3.5 transition-shadow hover:shadow-sm"
    >
