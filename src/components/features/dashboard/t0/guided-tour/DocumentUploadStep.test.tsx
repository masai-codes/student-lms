...
vi.mock('@/lib/api/dashboard/dashboardApi', () => ({
  fetchT0FlowDocuments: hoisted.fetchDocs,
}))
...
describe('DocumentUploadStep', () => {
  it('shows the done state when documents are already uploaded', async () => {
    hoisted.fetchDocs.mockResolvedValue({
      documentsUploaded: true,
      documentsVerified: false,
      admissionsFormUrl: 'https://sso/docs',
    })
    renderStep()
    await waitFor(() =>
      expect(screen.getByTestId('document-upload-done')).toBeTruthy(),
    )
  ...
  it('redirects to the admissions portal and refetches when not uploaded', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('document-upload-continue')).toBeTruthy(),
    )
