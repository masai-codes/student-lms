...
function defaultPendingTab(batch: BatchT0Status | undefined): TabKey {
  if (
    batch?.showProgramTab &&
    batch.lms.complete &&
    batch.program &&
    !batch.program.complete
  ) {
    return 'program'
  }
  return 'lms'
...
export function GuidedTourOverlay({
  ...
  const initialBatch =
    status.batches.find(
      (b) => b.batchId === (initialBatchId ?? status.batches.at(0)?.batchId),
    ) ?? status.batches.at(0)
  ...
  const [tab, setTab] = useState<TabKey>(
    initialTab ?? defaultPendingTab(initialBatch),
  )
  ...
  const selectedBatch: BatchT0Status | undefined =
    status.batches.find((b) => b.batchId === selectedBatchId) ??
    status.batches.at(0)
  ...
  const effectiveTab: TabKey =
    tab === 'program' && !programUnlocked ? 'lms' : tab
  ...
  const activeIndex = activeStep
    ...
    : 0
  const tabProgress =
    effectiveTab === 'lms' ? selectedBatch?.lms : selectedBatch?.program

  // Segment bar spans the tab's video steps; index of the active one among them.
  const videoStepKeys = steps
  ...
  const idCard =
    lectures && effectiveTab === 'program'
      ? getIdCardState(lectures, batchFlowVariant)
      : null
  ...
  const refetchProgress = () => {
    ...
    void queryClient.invalidateQueries({
      queryKey: ['dashboard', 't0-flow-lectures'],
    })
  }
  ...
  const selectBatch = (value: string) => {
    pushDashboardEvent(
      'l_dashboard_guided_tour_batch_select_id_' + Number(value),
      {
  ...
  const batchSelect =
    status.batches.length > 1 ? (
      <Select
        value={selectedBatch ? String(selectedBatch.batchId) : undefined}
        onValueChange={selectBatch}
      >
        <SelectTrigger
          aria-label="Batch"
          className="w-full"
          data-testid="guided-tour-batch-select"
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent className="z-[210]">
          {status.batches.map((b) => (
            <SelectItem
              ...
              data-testid={`guided-tour-batch-option-${b.batchId}`}
            >
              {b.batchName}
            </SelectItem>
          ))}
  ...
  const tabsRow = (
    <div className="flex gap-2" role="tablist" data-testid="guided-tour-tabs">
      ...
      >
        {programUnlocked ? null : (
          <Lock
            className="size-4"
            aria-hidden
            data-testid="guided-tour-tab-program-lock"
          />
        )}
        Program Onboarding
      </button>
    ...
    <>
      ...
      {showIdCard ? (
        <div className="mt-4">
          <IdCardStep
            url={idCard?.url ?? null}
            unlocked={idCard?.unlocked ?? false}
          />
        </div>
      ) : null}
    </>
...
const TAB_BASE =
  'inline-flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand'
const TAB_ACTIVE = `${TAB_BASE} border-brand bg-brand/10 text-brand`
const TAB_IDLE = `${TAB_BASE} border-border bg-surface text-foreground-muted hover:bg-surface-muted`
const TAB_LOCKED = `${TAB_BASE} cursor-not-allowed border-border bg-surface text-foreground-subtle`
