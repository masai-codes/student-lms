 ...
 * users. Two tabs — LMS Walkthrough (always) and Program Onboarding (unlocked
 * once full fees are paid) — each with a progress bar, a step list, and an
 * active panel (video or fixed action). Watching a video reports completion and
 * refetches progress so the bar advances live.
 */
export function GuidedTourOverlay({ status, onSeeDashboard }: GuidedTourOverlayProps) {
  const queryClient = useQueryClient()
  const primaryBatch: BatchT0Status | undefined = status.batches.at(0)
  const showProgramTab = primaryBatch?.showProgramTab ?? false

  const [tab, setTab] = useState<TabKey>('lms')
  const [activeKey, setActiveKey] = useState<string | null>(null)

  const { data: lectures } = useQuery({
    queryKey: ['dashboard', 't0-flow-lectures', primaryBatch?.batchId ?? null],
    queryFn: () => fetchT0FlowLectures(primaryBatch?.batchId),
    enabled: primaryBatch !== undefined,
  })

  const steps: Array<GuidedTourStep> = useMemo(() => {
    if (!lectures) return []
    return tab === 'lms' ? buildLmsSteps(lectures, status) : buildProgramSteps(lectures)
  }, [lectures, status, tab])

  const activeStep = steps.find((s) => s.key === activeKey) ?? steps.at(0)
  const tabProgress = tab === 'lms' ? primaryBatch?.lms : primaryBatch?.program
      ...
      <div className="mx-auto flex min-h-full max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6">
        ...
        <div className="flex gap-2" role="tablist" data-testid="guided-tour-tabs">
          <button
            type="button"
            role="tab"
            aria-selected={tab === 'lms'}
            onClick={() => selectTab('lms')}
            className={tab === 'lms' ? TAB_ACTIVE : TAB_IDLE}
          ...
          {showProgramTab ? (
            <button
              ...
              className={tab === 'program' ? TAB_ACTIVE : TAB_IDLE}
        ...
        <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
          ...
          <div className="min-w-0">
            {activeStep?.kind === 'video' && activeStep.video && primaryBatch ? (
              <GuidedTourVideoStep
                key={activeStep.key}
...
const TAB_ACTIVE = 'rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white'
const TAB_IDLE = 'rounded-lg px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100'
