...
export function IdCardStep({ url, unlocked }: IdCardStepProps) {
  if (!unlocked) {
    return (
      <div
        className="flex flex-col items-center gap-4 rounded-2xl bg-brand-subtle p-5 sm:flex-row sm:gap-6"
      ...
      >
        <div className="relative h-[130px] w-[200px] shrink-0 overflow-hidden rounded-xl border border-white/60 shadow-sm">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-surface/10">
            <span className="flex size-14 items-center justify-center rounded-full bg-brand shadow-md">
              <Lock
                size={26}
                weight="fill"
        ...
        <div className="text-center sm:text-left">
          <h3 className="text-lg font-bold text-foreground">ID Card</h3>
  ...
  if (!url) {
    return (
      <div className={CARD} data-testid="id-card-step">
        <IdentificationCard className="size-8 text-brand" aria-hidden />
        <p className="text-sm font-medium text-foreground">
          Your ID card is being generated
        </p>
        <p
          className="text-sm text-foreground-muted"
          data-testid="id-card-generating"
        >
          We&apos;re verifying your details — your ID card will be ready within
          30 minutes.
        </p>
      </div>
    )
    ...
    <div className={CARD} data-testid="id-card-step">
      ...
      {isPdf ? (
        <iframe
          ...
          className="h-[420px] w-full max-w-sm rounded-lg border border-border"
      ...
      <a
        ...
        onClick={() =>
          pushDashboardEvent('l_dashboard_guided_tour_id_card_download')
        }
        className="inline-flex h-11 items-center justify-center gap-2 self-center rounded-lg bg-brand px-5 text-sm font-semibold text-brand-foreground hover:opacity-90"
