...
beforeEach(() => {
  ...
  hoisted.save.mockResolvedValue({
    savedValues: VALID_VALUES,
    referenceNumber: 'TC-1-section_7',
  })
...
describe('AgreementStep', () => {
  ...
  it('shows the animated success screen once signed (no details, no PDF link)', () => {
    renderStep(
      section({ completed: true, agreementPdfUrl: 'https://s3/a.pdf' }),
    )
  ...
  it('records the first view and shows the days-remaining countdown', async () => {
    ...
    expect(screen.getByTestId('agreement-countdown').textContent).toContain(
      '7 days remaining',
    )
  ...
  it('counts down in hours when under a day remains in the review window', () => {
    renderStep(
      section({
        viewTime: '2026-01-01T00:00:00.000Z',
        daysLeft: 0,
        hoursLeft: 5,
  ...
  it('shows the location consent checkbox + read-only detected location', () => {
    ...
    expect(
      screen.getByTestId('agreement-location-value').textContent,
    ).toContain('Bengaluru')
  })

  it('blocks Continue and points to the enable-location guide until the box is checked', () => {
    renderStep(
      section({ savedValues: { ...VALID_VALUES, location: undefined } }),
    )
    ...
    expect(screen.getByTestId('agreement-continue-hint').textContent).toContain(
      'Location access is required',
    )
    fireEvent.click(screen.getByTestId('agreement-continue'))
    expect(screen.queryByTestId('agreement-pdf-viewer')).toBeNull()
    expect(
      screen.getByTestId('agreement-location-error').textContent,
    ).toContain('Please select the checkbox to fetch location')
  })
    ...
    renderStep(
      section({
        savedValues: {
          ...VALID_VALUES,
          name: '',
          parentsEmail: 'not-an-email',
        },
      }),
    )
    // Continue stays enabled so it can surface the errors (instead of a dead disabled button).
    expect(
      screen.getByTestId<HTMLButtonElement>('agreement-continue').disabled,
    ).toBe(false)
  ...
  it('reveals a detailed, labelled summary of every invalid field when Continue is pressed', () => {
    ...
    expect(
      screen.getByTestId('agreement-validation-summary-item-name').textContent,
    ).toContain('Name')
    expect(
      screen.getByTestId('agreement-validation-summary-item-name').textContent,
    ).toContain('required')
    expect(
      screen.getByTestId('agreement-validation-summary-item-parentsEmail')
        .textContent,
    ).toContain("Parent's Email ID")
    expect(
      screen.getByTestId('agreement-validation-summary-item-parentsEmail')
        .textContent,
  ...
  it('tells the learner to accept the document when Continue is disabled on a document step', async () => {
    ...
    expect(
      screen.getByTestId('agreement-continue').getAttribute('disabled'),
    ).not.toBeNull()
    expect(screen.getByTestId('agreement-continue-hint').textContent).toContain(
      'accept the document',
    )
  ...
  it('walks details → document → certificate → submit', async () => {
    ...
    await waitFor(() =>
      expect(hoisted.save).toHaveBeenCalledWith(
        7,
        expect.objectContaining({ name: 'Riya' }),
      ),
    )
    expect(screen.getByTestId('agreement-pdf-viewer')).toBeTruthy()
