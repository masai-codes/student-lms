 ...
 */
export function AgreementValidationSummary({ issues }: AgreementValidationSummaryProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-semibold text-red-800">
        <Warning size={18} weight="fill" className="shrink-0 text-red-600" aria-hidden />
        <span>
          Please fix {issues.length} {issues.length === 1 ? 'field' : 'fields'} to continue
        </span>
      </div>
