...
const kit = (over: Partial<StudentKitStatus> = {}): StudentKitStatus => ({
  applicable: true, detailsFilled: false, trackingUrl: null, trackingId: null, admissionsFormUrl: 'https://sso/kit', ...over,
})
...
describe('StudentKitStep', () => {
  it('offers the fill-details redirect when not filled', () => {
    ...
    expect(window.open).toHaveBeenCalledWith('https://sso/kit', '_blank', 'noopener,noreferrer')
  })
  ...
  it('shows the tracking link once the kit ships', () => {
    render(<StudentKitStep kit={kit({ detailsFilled: true, trackingUrl: 'https://track/1' })} />)
    expect(screen.getByTestId<HTMLAnchorElement>('student-kit-track').getAttribute('href')).toBe('https://track/1')
  })
})
