...
describe('OnboardingStepsBanner', () => {
  it('renders nothing when there are no banners', () => {
    const { container } = render(
      <OnboardingStepsBanner banners={[]} onResume={vi.fn()} />,
    )
  ...
  it('features the remaining-steps count and shows the fraction + title for a single course', () => {
    ...
    expect(
      screen.getByTestId('dashboard-onboarding-banner-title').textContent,
    ).toContain('MERN')
    // 2 of 5 done → 3 remaining is the hero count; fraction stays as context.
    expect(
      screen.getByTestId('dashboard-onboarding-banner-count').textContent,
    ).toBe('3steps left')
    expect(
      screen.getByTestId('dashboard-onboarding-banner-progress').textContent,
    ).toContain('2/5')
  ...
  it('singularizes the count when only one step is left', () => {
    render(
      <OnboardingStepsBanner
        banners={[banner({ completed: 4, total: 5 })]}
        onResume={vi.fn()}
      />,
    )
    expect(
  ...
  it('resumes on the course + tab the banner points at', () => {
    ...
    render(
      <OnboardingStepsBanner
        banners={[banner({ batchId: 9, targetTab: 'program' })]}
        onResume={onResume}
      />,
    )
    ...
    render(
      <OnboardingStepsBanner
        banners={[
          ...
          banner({ batchId: 6, courseTitle: 'Data Analytics' }),
        ]}
        onResume={vi.fn()}
      />,
    )
