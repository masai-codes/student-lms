...
const timer = (
  ...
  paymentUrl:
    over.paymentUrl === undefined ? 'https://pay.test/x' : over.paymentUrl,
...
describe('FeePaymentBanners', () => {
  ...
  it('timer: shows the course name, days pill, active CTA, and no dots for one batch', () => {
    ...
    expect(
      screen
        .getByTestId('dashboard-fee-payment-banner')
        .getAttribute('data-variant'),
    ).toBe('timer')
    expect(screen.getByTestId('dashboard-fee-payment-course').textContent).toBe(
      'MERN',
    )
    expect(screen.getByText(/Pay your remaining program fee/)).toBeTruthy()
    expect(screen.getByTestId('dashboard-fee-payment-days').textContent).toBe(
      '7 days remaining',
    )
    expect(
      screen.getByTestId('dashboard-fee-payment-cta').getAttribute('href'),
    ).toBe('https://pay.test/x')
  ...
  it('timer: shows hours remaining when less than a day is left', () => {
    render(
      <FeePaymentBanners
        banners={[timer({ daysRemaining: 0, hoursRemaining: 5 })]}
      />,
    )
    expect(screen.getByTestId('dashboard-fee-payment-days').textContent).toBe(
      '5 hours remaining',
  ...
  it('renders one slide + one dot per batch when there are multiple', () => {
    ...
    expect(screen.getAllByTestId('dashboard-fee-payment-banner')).toHaveLength(
      2,
    )
    expect(
      screen
        .getByTestId('dashboard-fee-payment-dots')
