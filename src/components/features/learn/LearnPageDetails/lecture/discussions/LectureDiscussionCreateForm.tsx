...
type LectureDiscussionCreateFormProps = {
  className?: string
  disabled?: boolean
  onSubmit?: (payload: { title: string; descriptionMarkdown: string }) => void | Promise<void>
}
...
export function LectureDiscussionCreateForm({
  ...
  return (
    <form
      className={cn('space-y-4 rounded-xl border border-gray-200 bg-white p-4', className)}
      onSubmit={e => {
        e.preventDefault()
        void handleSubmit()
      }}
