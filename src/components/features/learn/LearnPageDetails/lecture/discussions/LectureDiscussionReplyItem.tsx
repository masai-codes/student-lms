...
export function LectureDiscussionReplyItem({ thread }: LectureDiscussionReplyItemProps) {
  const authorName = thread.author?.name?.trim() || 'Student'
  const avatarUrl = discussionAvatarUrl(authorName, thread.authorProfileImageUrl)
  return (
    <article className="flex gap-3 border-t border-gray-100 py-3 first:border-t-0 first:pt-0">
