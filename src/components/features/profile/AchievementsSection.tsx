    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 z-10 text-foreground-subtle hover:text-foreground-muted transition-colors focus-visible:outline-none"
          aria-label="Close"
        >
          <X size={20} />
        </button>
        ...
        <div className="flex justify-center pt-10 pb-6 px-6 bg-[linear-gradient(160deg,#e0f2fe_0%,#f0fdf4_50%,#faf5ff_100%)] dark:bg-[linear-gradient(160deg,rgb(14_165_233_/_0.12)_0%,rgb(34_197_94_/_0.10)_50%,rgb(168_85_247_/_0.12)_100%)]">
          ...
          >
            {badge.image ? (
              ...
              <div className="size-full bg-surface-muted flex items-center justify-center text-6xl rounded-xl">
                🏅
              </div>
            )}
          </div>
        ...
        <div className="px-6 pb-8 flex flex-col items-center gap-3 text-center">
          ...
          {!isLocked && formattedDate ? (
            <span className="inline-flex items-center px-4 py-1.5 rounded-full border border-brand text-brand text-sm font-medium">
...
function BadgeCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="relative flex flex-col items-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-lg"
      aria-label={badge.title}
    >
      <div
        className={`relative size-28 rounded-lg overflow-visible shrink-0 ${isLocked ? 'opacity-50' : ''}`}
      >
        {badge.image ? (
          <img
            src={badge.image}
          ...
          <div className="size-full bg-surface-muted flex items-center justify-center rounded-lg border border-border">
      ...
      <div className="rounded-2xl border border-border bg-surface p-6">
        ...
        <div className="flex flex-wrap gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="size-28 rounded-lg bg-surface-muted animate-pulse"
            />
          ))}
        </div>
      </div>
