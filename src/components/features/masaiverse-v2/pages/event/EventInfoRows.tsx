...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="flex size-11 shrink-0 items-center justify-center rounded-[12px] border border-border bg-surface-muted text-foreground-muted">
        {icon}
      </span>
      <span className="min-w-0">
        ...
        {subtitle ? (
          <span className="block truncate text-[13px] leading-5 text-foreground-muted">
            {subtitle}
          </span>
        ) : null}
      </span>
...
export default function EventInfoRows({ event }: EventInfoRowsProps) {
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <div className="flex items-center gap-3">
        <span className="flex size-11 shrink-0 flex-col items-center justify-center overflow-hidden rounded-[12px] border border-border bg-surface leading-none">
          {badge ? (
            <>
              <span className="flex h-[14px] w-full items-center justify-center bg-accent-warm text-[8px] font-bold text-accent-warm-foreground">
                {badge.month}
              </span>
              <span className="flex flex-1 items-center text-[16px] font-bold text-foreground">
                {badge.day}
              </span>
            </>
          ) : null}
        ...
        <span className="min-w-0">
          ...
          {timeLine ? (
            <span className="block text-[13px] leading-5 text-foreground-muted">
              {timeLine}
            </span>
          ) : null}
        </span>
