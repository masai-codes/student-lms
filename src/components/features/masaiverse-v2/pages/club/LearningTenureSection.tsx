      ...
      <ResponsiveCardCarousel
        ...
        renderItem={(item) => (
          <div className="flex h-full flex-col rounded-[16px] border border-[#EDEAE8] bg-white p-5">
            ...
            <p className="mt-4 text-[16px] font-bold leading-5 text-[#111827]">
              {item.heading}
            </p>
            {item.text ? (
              <p className="mt-1.5 flex-1 text-[14px] leading-5 text-[#6B7280]">
                {item.text}
              </p>
            ) : null}
            {item.tags.length > 0 ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {item.tags.map((tag, tagIndex) => (
                  <span
                    key={`${tag}-${tagIndex}`}
                    className="rounded-full bg-masaiverse-orange/10 px-2.5 py-1 text-[12px] font-semibold text-masaiverse-orange"
                  >
                    {tag}
                  </span>
                ))}
