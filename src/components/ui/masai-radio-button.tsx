...
export function MasaiRadioButton({
  ...
  const labelClassName =
    size === 'large'
      ? 'type-b1-regular text-foreground'
      : 'type-b2-regular text-foreground'
    ...
    >
      <label className="inline-flex cursor-pointer items-start gap-2">
        <RadioGroupPrimitive.Item
          value="selected"
          className={cn(
            'rounded-full border border-brand bg-surface outline-none focus-visible:ring-2 focus-visible:ring-primary-400 disabled:cursor-not-allowed disabled:opacity-60',
            root,
          )}
        >
          <RadioGroupPrimitive.Indicator className="flex items-center justify-center">
            <span className={cn('rounded-full bg-brand', dot)} />
