      ...
      if (href && AUDIO_EXTENSIONS.test(href)) {
        return (
          <audio
            controls
            src={href}
            className="w-full min-w-[300px] rounded-lg mt-1"
          />
        )
      }
      return (
...
function createCardComponents(): Components {
  return {
    ...
    h1: ({ node, ...props }) => (
      <h1
        className="mt-6 mb-3 text-[28px] leading-[34px] font-semibold text-foreground"
        {...props}
      />
    ),
    h2: ({ node, ...props }) => (
      <h2
        className="mt-6 mb-3 text-[24px] leading-[30px] font-semibold text-foreground"
        {...props}
      />
    ),
    h3: ({ node, ...props }) => (
      <h3
        className="mt-5 mb-2 text-[38px] leading-[46px] font-semibold text-foreground"
        {...props}
      />
    ),
    h4: ({ node, ...props }) => (
      <h4
        className="mt-4 mb-2 text-[18px] leading-[24px] font-semibold text-foreground"
        {...props}
      />
    ),
    h5: ({ node, ...props }) => (
      <h5
        className="mt-4 mb-2 text-[16px] leading-[22px] font-semibold text-foreground"
        {...props}
      />
    ),
    h6: ({ node, ...props }) => (
      <h6
        className="mt-3 mb-2 text-[14px] leading-[20px] font-semibold text-foreground"
        {...props}
      />
    ),
...
function createDetailComponents(): Components {
  return {
    ...
    h1: ({ children }: { children?: ReactNode }) => (
      <h1 className="type-h4 mb-3 text-foreground">{children}</h1>
    ),
    h2: ({ children }: { children?: ReactNode }) => (
      <h2 className="type-h5 mb-3 text-foreground">{children}</h2>
    ),
    h3: ({ children }: { children?: ReactNode }) => (
      <h3 className="type-h6 mb-2 text-foreground">{children}</h3>
    ),
  }
}
